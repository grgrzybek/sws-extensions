...
public class SweJaxbUnmarshaller implements Unmarshaller {
	...
	private <T> JAXBElement<T> unmarshal0(XMLEventReader reader, Class<T> declaredType) throws JAXBException {
		try {
			...
			ElementPattern<T> pattern = null;

			if (declaredType == null) {
				// we can unmarshal to @XmlRootElement annotated classes ONLY
				pattern = this.<T>determineRootXmlPattern(unmarshalledElement);
				if (pattern == null)
					throw new UnmarshalException("Can't unmarshall \"" + unmarshalledElement.getName()
							+ "\" element. Ensure there's proper @XmlRootElement annotated class in this unmarshaller's context.");
			}
			else {
				...
				TypedPattern<T> typeDefinitionComponentPattern = (TypedPattern<T>) this.jaxbContext.patterns.get(declaredType);
				if (typeDefinitionComponentPattern == null)
					throw new UnmarshalException("Can't unmarshall \"" + unmarshalledElement.getName() + "\" into \"" + declaredType.getName()
							+ "\". There's no mapping of this class to proper content model in this context.");

				// we'll wrap it inside ElementPattern which will peel the events off of the start and end elemen events
				pattern = ElementPattern.newElementPattern(unmarshalledElement.getName(), typeDefinitionComponentPattern);
			}

			UnmarshallingContext context = new UnmarshallingContext();

			// go!
			T result = pattern.consume(reader, context);
	...
	private void positionReaderAtStartElement(XMLEventReader reader) throws UnmarshalException {
		...
		try {
			while (true) {
				...
				if (first.getEventType() == XMLStreamConstants.START_ELEMENT)
					break;
	 ...
	 */
	@SuppressWarnings("unchecked")
	private <T> ElementPattern<T> determineRootXmlPattern(StartElement startElementEvent) throws MarshalException {
		...
		return (ElementPattern<T>) this.jaxbContext.rootPatternsForQNames.get(qName);
	}
